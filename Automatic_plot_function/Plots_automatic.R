    ...
    if("Alpha" %in% colnames(chr.list[[i]]) && "Beta" %in% colnames(chr.list[[i]])){
      plot.list[[i]] <- 
        ...
        geom_rect(aes(xmin = 0, xmax = 6000, ymin = -Inf, ymax = Inf),
                  fill = "tan", alpha = 0.01)+
        geom_line((aes(x=Window_pb, y=Alpha*100, colour="Alpha"))) +
        geom_errorbar(aes(x=Window_pb, ymin=Alpha*100-Alpha.Error*100, ymax=Alpha*100+Alpha.Error*100, colour="Alpha"), width=0.7, size=0.4, position=position_dodge(0.05))+
        #geom_point((aes(x=Window_pb, y=Alpha*100)), size=2, shape=21, fill="white")+
        geom_line((aes(x=Window_pb, y=Beta*100, colour="Beta"))) +
        geom_errorbar(aes(x=Window_pb, ymin=Beta*100-Beta.Error*100, ymax=Beta*100+Beta.Error*100, colour="Beta"), width=0.7, size=0.4, position=position_dodge(0.05))+
  ...
  for (i in 1:length(chr.list)) {
    if("Observed.Steady.State" %in% colnames(chr.list[[i]]))
    {
      plot.list[[i]] <- 
        ...
        geom_rect(aes(xmin = 0, xmax = 6000, ymin = -Inf, ymax = Inf),
                  fill = "tan", alpha = 0.01)+
        geom_line((aes(x=Window_pb, y=Observed.Steady.State))) +
        scale_x_continuous(breaks= seq(-2000,8000,by=500),
                           limits = c(-2000, 8000),
                           label = c("-2000bp", "", "-1000bp", "", "0%", "", "16%", "", "33%", "", "50%", "","66%", "", "83%", "", "100%", "", "1000bp", "", "2000bp"))+
        annotate("text", x=-1000, y=0, label= "Upstream", size = 6, colour="darkgrey", alpha = 0.7)+
  ...
  for (i in 1:length(chr.list)) {
    if("Predicted.Steady.State" %in% colnames(chr.list[[i]]))
    {
      plot.list[[i]] <- 
        ...
        geom_rect(aes(xmin = 0, xmax = 6000, ymin = -Inf, ymax = Inf),
                  fill = "tan", alpha = 0.01)+
        geom_line((aes(x=Window_pb, y=Predicted.Steady.State*100))) +
        scale_x_continuous(breaks= seq(-2000,8000,by=500),
                           limits = c(-2000, 8000),
                           label = c("-2000bp", "", "-1000bp", "", "0%", "", "16%", "", "33%", "", "50%", "","66%", "", "83%", "", "100%", "", "1000bp", "", "2000bp"))+
        annotate("text", x=-1000, y=0, label= "Upstream", size = 6, colour="darkgrey", alpha = 0.7)+
  ...
  for (i in 1:length(chr.list)) {
    if("X1.2....Alpha...Beta." %in% colnames(chr.list[[i]]))
    {
      plot.list[[i]] <- ggplot(chr.list[[i]])+
        geom_rect(aes(xmin = 0, xmax = 6000, ymin = -Inf, ymax = Inf),
                  fill = "tan", alpha = 0.01)+
        geom_line((aes(x=Window_pb, y=X1.2....Alpha...Beta.))) +
        scale_x_continuous(breaks= seq(-2000,8000,by=500),
                           limits = c(-2000, 8000),
                           label = c("-2000bp", "", "-1000bp", "", "0%", "", "16%", "", "33%", "", "50%", "","66%", "", "83%", "", "100%", "", "1000bp", "", "2000bp"))+
        annotate("text", x=-1000, y=0, label= "Upstream", size = 6, colour="darkgrey", alpha = 0.7)+
